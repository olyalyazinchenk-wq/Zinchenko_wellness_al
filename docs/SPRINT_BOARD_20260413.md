...
- remove `РѕС‚ 2 990 в‚Ѕ` from the live-adjacent mini-app surface because it conflicts with current pilot pricing
...
- treat the recovered failures on `2026-05-02 15:09:39-15:17:57 MSK` and `20:26:15-20:27:14 MSK` as live evidence of fragility
...
- use the delivered `week` follow-up (`"РЇ СЃРґР°Р»Р° Р°РЅР°Р»РёР·С‹"`) as the only live trigger for the next premium-upgrade design
...
- harden delivery truth first, classify the same-user branch stack second, neutralize the mini-app third, make polling fallback explicit fourth, then run one fresh premium-upgrade brief
...
7. Remove `РѕС‚ 2 990 в‚Ѕ` from `mini-app/index.html`.
...
9. Verify whether bot polling should depend on the local proxy at `127.0.0.1:12334`, and add a stable no-proxy fallback if not.
10. Extend `sanitize_live_reply()` and benchmark checks for invented names, over-familiar tone, and early diagnosis-like labels.
11. Rerun the quality benchmark only after steps `1-9` land and confirm model reach stays near the current `9/20` baseline.
12. Create exactly one premium-upgrade brief from the delivered `week` follow-up and new labs.
13. Keep Telegram-first operations, manual concierge payment, and official prices at `3900 / 6900 / 14900 RUB`.
