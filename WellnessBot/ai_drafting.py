...
def route_live_reply(user_text: str) -> str | None:
    ...
    if contains_any(normalized, ("как у вас устроен", "разбор у вас", "что я получу", "что получу", "на выходе", "список добавок", "именно так и работаете")):
        return (
            ...
            "Когда нужен более глубокий и собранный результат, подключается персональное досье: это уже структурированный документ "
    ...
    if contains_any(normalized, ("премиальное досье", "премиум", "досье", "в какой момент становится нужен")):
        return (
            "Персональное досье нужно в момент, когда после первой ясности уже хочется не просто поговорить, а собрать кейс в цельную рабочую картину.\n\n"
            ...
            "То есть сначала Telegram помогает убрать хаос, а персональное досье нужно там, где требуется уже собранный премиальный результат."
        )
